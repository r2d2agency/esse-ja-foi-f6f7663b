...
export async function getRelatoriosGerais(filtros: { dataInicio?: string | null; dataFim?: string | null }) {
  ...
  const stats = await db.execute(sql`
    SELECT
      ...
      (SELECT count(*)::int FROM leiloes WHERE status = 'ENCERRADO' ${andWhereClause}) as leiloes_realizados,
      (SELECT count(*)::int FROM negociacoes WHERE status = 'CONCLUIDA' ${andWhereClause}) as vendas_concluidas,
      (SELECT COALESCE(SUM(valor_venda), 0)::numeric FROM negociacoes WHERE status = 'CONCLUIDA' ${andWhereClause}) as volume_vendido,
      (SELECT COALESCE(SUM(valor_comissao), 0)::numeric FROM negociacoes WHERE status = 'CONCLUIDA' ${andWhereClause}) as comissao_gerada,
      (SELECT count(*)::int FROM profiles WHERE role = 'comprador' AND ativo = true ${andWhereClause}) as compradores_ativos,
      (SELECT count(*)::int FROM profiles WHERE role = 'vendedor' ${andWhereClause}) as vendedores_cadastrados
  `);

  const funnel = await db.execute(sql`
    SELECT
      (SELECT count(*)::int FROM veiculos ${whereClause}) as cadastrados,
      (SELECT count(*)::int FROM veiculos WHERE status_analise = 'AGUARDANDO_ANALISE' ${andWhereClause}) as em_analise,
      ...
      (SELECT count(*)::int FROM veiculos WHERE status_analise = 'PRONTO_PARA_ANUNCIO' ${andWhereClause}) as prontos_anuncio,
      (SELECT count(*)::int FROM anuncios_veiculo ${whereClause}) as publicados,
      (SELECT count(DISTINCT leilao_id)::int FROM lances ${whereClause}) as com_lances,
